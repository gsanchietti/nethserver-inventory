...
if Facter.value(:kernel) == 'Linux'
  ...
  Facter::Util::Resolution.exec('cat /proc/mounts 2> /dev/null').each_line do |line|
    ...
    available = Facter::Core::Execution.exec('/bin/df '+ mount +' --total | tail -1 | awk \'{print $2}\'')
    used = Facter::Core::Execution.exec('/bin/df '+ mount +' --total | tail -1 | awk \'{print $3}\'')
    mounts[mount]['used_bytes'] = Integer(used) * 1024
    mounts[mount]['size_bytes'] = Integer(available) * 1024
  end
